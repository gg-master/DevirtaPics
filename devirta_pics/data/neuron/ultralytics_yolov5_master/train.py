...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if RANK in [-1, 0]:
        ...
        callbacks.to_qt_format('on_pretrain_routine_end')
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if RANK in [-1, 0]:
                ...
                callbacks.to_qt_format('on_train_batch_end', ni, model, imgs, targets, paths, plots, opt.sync_bn)
        ...
        if RANK in [-1, 0]:
            # mAP
            callbacks.to_qt_format('on_train_epoch_end', epoch=epoch)
            ema.update_attr(model, include=['yaml', 'nc', 'hyp', 'names', 'stride', 'class_weights'])
            final_epoch = (epoch + 1 == epochs) or stopper.possible_stop
            if not noval or final_epoch:  # Calculate mAP
                results, maps, _ = val.to_qt_format(data_dict,
            ...
            callbacks.to_qt_format('on_fit_epoch_end', log_vals, epoch, best_fitness, fi)
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ...
                del ckpt
                callbacks.to_qt_format('on_model_save', last, epoch, final_epoch, best_fitness, fi)
    ...
    if RANK in [-1, 0]:
        ...
        for f in last, best:
            if f.exists():
                ...
                if f is best:
                    LOGGER.info(f'\nValidating {f}...')
                    results, _, _ = val.to_qt_format(data_dict,
                                                     batch_size=batch_size // WORLD_SIZE * 2,
                                                     imgsz=imgsz,
                    ...
                    if is_coco:
                        callbacks.to_qt_format('on_fit_epoch_end', list(mloss) + list(results) + lr, epoch, best_fitness, fi)

        callbacks.to_qt_format('on_train_end', last, best, plots, epoch, results)
